#!/bin/bash
# Script pour générer le plan Terraform via Docker (WSL) - Exercices
# Usage: ./scripts/wsl/terraform-plan.sh [exercice_dir]
...
if [ $exit_code -eq 0 ]; then
    echo -e "${GREEN}✅ Plan généré avec succès${NC}"
else
    echo -e "${RED}❌ Erreur lors de la génération du plan${NC}"
fi
